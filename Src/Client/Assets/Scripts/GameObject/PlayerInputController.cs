...
public class PlayerInputController : MonoBehaviour {
    ...
    private IEnumerator BeginNav(Vector3 target)
    {
        ...
        yield return null;
        autoNav = true;
        if (!state.Equals(SkillBridge.Message.CharacterState.Move))
        {
    ...
    public void StopNav()
    {
        autoNav = false;
        agent.ResetPath();
        if (!state.Equals(SkillBridge.Message.CharacterState.Idle)){
            state = SkillBridge.Message.CharacterState.Idle;
    ...
    public void NavMove()
    {
        ...
        if (!agent.pathStatus.Equals(NavMeshPathStatus.PathComplete)) return;
    ...
    public void SendEntityEvent(EntityEvent entityEvent,int param = 0)
    {
        ...
        if (character != null)
        {
            Services.MapService.Instance.SendEntitySync(character.EntityData,entityEvent, param);
        }
    }
}
